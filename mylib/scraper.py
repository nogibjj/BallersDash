...
class Scraper:
    ...
    def get_all_games_current_season(self, season_start_date, season_end_date):
        ...
        while cur_dt <= self.date_formatter(season_end_date):
            link = stat_link.format(year=cur_dt.year, month=self.month_dict[cur_dt.month].lower())
    ...
    def get_team_roster(year, team_abv):
        stat_link = BasketBallReferenceLinks.team_roster.format(year=year, team_abv=team_abv)
        return pd.read_html(stat_link)[0]

    def get_player_game_log(self, suffix__, year):
        stat_link = BasketBallReferenceLinks.player_game_log.format(suffix=suffix__, year=year, type='pgl_basic')
        try:
    ...
    def get_player_suffix(self, name):
        short_cut = {'Clint Capela': r'/c/capelca01', 'Enes Freedom': '/k/kanteen01',
                     'C.J. Miles': '/m/milescj01', 'Dennis Schröder': '/s/schrode01',
                     'P.J. Washington': '/w/washipj01', 'Nikola Vučević': '/v/vucevni01',
                     'David Duke Jr.': '/d/dukeda01.', 'Cedi Osman': '/o/osmande01',
                     'Dāvis Bertāns': '/b/bertada01', 'Luka Dončić': '/d/doncilu01',
                     'Maxi Kleber': '/k/klebima01', 'Frank Ntilikina': '/n/ntilila01',
                     'Vlatko Čančar': '/c/cancavl01', 'Nikola Jokić': '/j/jokicni01',
                     'D.J. Augustin': '/a/augusdj01', 'Alperen Şengün': '/s/sengual01',
                     'T.J. McConnell': '/m/mccontj01', 'P.J. Tucker': '/t/tuckepj01',
                     "D'Angelo Russell": '/r/russeda01', 'Didi Louzada': '/l/louzama01',
                     'R.J. Hampton': '/h/hamptrj01', 'B.J. Johnson': '/j/johnsbj01',
                     'M.J. Walker': '/w/walkemj01', 'D.J. Wilson': '/w/wilsodj01',
