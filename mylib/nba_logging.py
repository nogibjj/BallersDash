...
def load_data(seaso_n, season__type, toda_y, start, end):
    ...
    easter_teams_, western_teams_, team_log_dict_ = team_ranker(teams_df, seaso_n, season__type, {})
...
def h_2_h_helper(all_scores_: list, m: float, s_: float, col: str, team_: str, pcdf, id_, p=0):
    ...
    p.line(x=[max_pts, max_pts], y=[0, max_pcdf], line_width=2, line_color=col, line_dash='dashed')
    p.line(x=[min_pts, min_pts], y=[0, max_pcdf], line_width=2, line_color=col, line_dash='dashed')
    hover = HoverTool(tooltips=[("Points", "@x"), ("Probability %", "@y{0.00}")], mode='vline')
    p.add_tools(hover)

    return p


def h_2h_pdf_plot_helper(all_scores_: list, m: float, s_: float, col: str, team_: str, id_: str, p=0) -> figure:
    pdf = [p * 100 for p in norm.pdf(all_scores_, m, s_)]
    return h_2_h_helper(all_scores_, m, s_, col, team_, pdf, id_, p)


def h_2h_cdf_plot_helper(all_scores_: list, m: float, s_: float, col: str, team_: str, id_: str, p=0) -> figure:
...
def scaled_plot_helper(m1_, s1_, m2_, s2_, all_scores_, team_1, team_2, id_):
    szn_team_1_graph = h_2h_pdf_plot_helper(all_scores_, m1_, s1_, 'black', team_1, f'PDF {id_}')
    szn_team_2_graph_pdf = h_2h_pdf_plot_helper(all_scores_, m2_, s2_, 'red', team_2, f'PDF {id_}', szn_team_1_graph)
    ...
    szn_team_1_graph = h_2h_cdf_plot_helper(all_scores_, m1_, s1_, 'black', team_1, f'CDF {id_}')
    szn_team_2_graph_cdf = h_2h_cdf_plot_helper(all_scores_, m2_, s2_, 'red', team_2, f'CDF {id_}', szn_team_1_graph)
...
if __name__ == '__main__':
    ...
    with col4:
        options = st.multiselect('Game Types', ['Regular Season', 'Playoffs'], ['Regular Season', 'Playoffs'])
    ...
    for game in games:
        ...
        if st.checkbox(f"{team__1} VS {team__2}"):
            ...
            head_to_head_df.iloc[-1]['pts_diff_skew'] = data['Last head to head point differences skew']
            head_to_head_df.iloc[-1]['pts_diff_kurtosis'] = data['Lst head to head point differences kurtosis']
            ...
            head_to_head_stats_df = pd.DataFrame({f'{team__1}': [data[ii]['mean_1'], data[ii]['std_1'],
                                                                 ...
                                                                 data[ii]['kurtosis']],
                                                  f'{team__2}': [data[ii]['mean_2'], data[ii]['std_2'],
                                                                 data[ii]['median_2'],
                                                                 data[ii]['skew_2'], data[ii]['kurtosis_2']]},
                                                 index=['mean', 'std', 'median', 'skew', 'kurtosis'])

            hist = [f'{team__1} Season pts scaled mean', f'{team__2} Season pts scaled mean',
                    f'{team__1} Last 15 pts scaled mean', f'{team__2} Last 15 pts scaled mean',
                    f'{team__1} Last 10 pts scaled mean', f'{team__2} Last 10 pts scaled mean',
                    f'{team__1} Last 5 pts scaled mean', f'{team__2} Last 5 pts scaled mean']
            ...
            if main_tab == 'Head to Head':
                ...
                h_2_h_team_1_graph = h_2h_pdf_plot_helper(all_scores, m1, s1, 'black', team__1, 'PDF')
                h_2_h_team_2_graph_pdf = h_2h_pdf_plot_helper(all_scores, m2, s2, 'red', team__2, 'PDF',
                                                              h_2_h_team_1_graph)
                ...
                h_2_h_team_1_graph = h_2h_cdf_plot_helper(all_scores, m1, s1, 'black', team__1, 'CDF')
                h_2_h_team_2_graph_cdf = h_2h_cdf_plot_helper(all_scores, m2, s2, 'red', team__2, 'CDF',
                                                              h_2_h_team_1_graph)
            ...
            pp0 = figure(title="Pts Diff", x_axis_label='Date', y_axis_label='Points', x_axis_type='datetime')
            xx = pp0.line('GAME_DATE', 'PTS_DIFF', line_width=2, legend_label=f'PTS_DIFF', line_color='blue',
                          source=srcc)
            pp0.scatter('GAME_DATE', 'PTS_DIFF', line_width=4, line_color='blue', source=srcc)
            hov_ = HoverTool(mode='vline', tooltips=[("Date", "@dt}"), ("Points", "@PTS_DIFF")], renderers=[xx])
            pp0.add_tools(hov_)

            pp = figure(title="Raw Pts", x_axis_label='Date', y_axis_label='Points', x_axis_type='datetime')
            xx = pp.line('GAME_DATE', 'PTS_1', line_width=2, legend_label=f'PTS_{team__1}', line_color='black',
                         source=srcc)
            yy = pp.line('GAME_DATE', 'PTS_2', line_width=2, legend_label=f'PTS_{team__2}', line_color='red',
                         source=srcc)
            pp.scatter('GAME_DATE', 'PTS_1', line_width=4, line_color='black', source=srcc)
            pp.scatter('GAME_DATE', 'PTS_2', line_width=4, line_color='red', source=srcc)
            hov_ = HoverTool(mode='vline', tooltips=[("Date", "@dt}"), (f"{team__1}", "@PTS_1"),
                                                     (f"{team__2}", "@PTS_2")], renderers=[xx, yy])
            pp.add_tools(hov_)
            ...
            if main_tab == 'Head to Head':
                ...
                with col1:
                    ...
                    scaled_plot_helper(m1, s1, m2, s2, all_scores, team__1, team__2, 'All Season')
                    ...
                    scaled_plot_helper(m1, s1, m2, s2, all_scores, team__1, team__2, 'Last 10 Games')
                ...
                with col2:
                    ...
                    scaled_plot_helper(m1, s1, m2, s2, all_scores, team__1, team__2, 'Last 15 Games')
                    ...
                    scaled_plot_helper(m1, s1, m2, s2, all_scores, team__1, team__2, 'Last 5 Games')
