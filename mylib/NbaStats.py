...
def get_team_game_log(team_id, season, season_type):
    ...
    season_type.reverse()
    df = pd.concat([teamgamelog.TeamGameLog(season=season,
                                            season_type_all_star=[s_],
                                            team_id=team_id).get_data_frames()[0] for s_ in season_type])
...
def get_vs_points(team_1_df, team_2_df):
    ...
    df = pd.concat([df_1[['GAME_DATE', 'MATCHUP', 'WL', 'PTS']], df_2[['WL', 'PTS']]], axis=1)
...
def get_pts_stats(df, last_n_games=1000, mode=1):
    ...
    if mode == 1:
        ...
        return {'mean': mean, 'std': stdv, 'median': median, 'skew': skew_, 'kurtosis': kurtosis_}
    else:
        ...
        return {'mean_1': mean_1, 'std_1': stdv_1, 'median_1': median_1, 'mean_2': mean_2, 'std_2': stdv_2,
                'median_2': median_2, 'mean_diff': mean_3, 'std_diff': stdv_3, 'median_diff': median_3,
                'skew_1': skew_1, 'kurtosis': kurtosis_1, 'skew_2': skew_2, 'kurtosis_2': kurtosis_2}


def team_ranker(team_df, season, season_type, team_dict, mode=0, max_date=datetime(year=2050, month=1, day=1)):
    cols = ['W_PCT', 'FG_PCT', 'FG3_PCT', 'FTA', 'FT_PCT', 'OREB', 'DREB', 'AST', 'STL', 'BLK', 'TOV', 'PTS']
    ...
    res.columns = index_
    easter_teams = ['MIA', 'MIL', 'BOS', 'PHI', 'TOR', 'CHI', 'BKN', 'CLE', 'ATL', 'CHA', 'NYK', 'WAS', 'IND', 'DET',
                    'ORL']
    western_teams = ['PHX', 'MEM', 'GSW', 'DAL', 'UTA', 'DEN', 'MIN', 'LAC', 'NOP', 'SAS', 'LAL', 'SAC', 'POR', 'OKC',
                     'HOU']
    easter_teams = res[easter_teams]
    western_teams = res[western_teams]

    easter_teams = easter_teams.div(easter_teams.max(axis=1), axis=0).multiply(metric_wights, axis=0).sum()
    easter_teams = (easter_teams / easter_teams.abs().max()) * 100 * 0.98

    western_teams = western_teams.div(western_teams.max(axis=1), axis=0).multiply(metric_wights, axis=0).sum()
    western_teams = (western_teams / western_teams.abs().max()) * 100 * 0.98

    return easter_teams, western_teams, team_log_dict


def get_stats(team_1, team_2, team_log_dict_, to_merge_df, alternate_source, team_names):
    ...
    team_1_log = team_1_log.merge(to_merge_df, left_on='opp', right_on='index', how='left')
    team_2_log = team_2_log.merge(to_merge_df, left_on='opp', right_on='index', how='left')
    ...
    team_1_full_name = team_names.loc[team_names.abbreviation == team_1].full_name.iloc[0]
    team_2_full_name = team_names.loc[team_names.abbreviation == team_2].full_name.iloc[0]
    vs_df_2 = alternate_source.loc[((alternate_source['Visitor/Neutral'] == team_1_full_name) &
    ...
    try:
        ...
        vs_df_2['PTS_1'] = vs_df_2.apply(lambda r: r['PTS.1'] if r['Home/Neutral'] == team_1_full_name else r['PTS'],
                                         axis=1)
        vs_df_2['PTS_2'] = vs_df_2.apply(lambda r: r['PTS.1'] if r['Home/Neutral'] == team_2_full_name else r['PTS'],
                                         axis=1)
        vs_df_2['PTS_DIFF'] = vs_df_2['PTS_1'] - vs_df_2['PTS_2']
        vs_df_2['WL_1'] = vs_df_2.apply(lambda r: 'W' if r['PTS_DIFF'] > 0 else 'L', axis=1)
        vs_df_2['WL_2'] = vs_df_2.apply(lambda r: 'W' if r['PTS_DIFF'] < 0 else 'L', axis=1)
        ...
        vs_df_2 = vs_df_2.loc[~vs_df_2.GAME_DATE.isin(vs_df.GAME_DATE.values)][vs_df_2['PTS'].notna()]
        ...
        win_margin_vs = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]

        dm = vs_pts_stats['mean_1'] + vs_pts_stats['mean_2']
        ds = vs_pts_stats['std_1'] + vs_pts_stats['std_2']
        total_points_vs = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]
    except ValueError:
    ...
    total_points_lst_5 = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]
    ...
    total_points_lst_10 = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]
    ...
    total_points_lst_15 = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]

    dm = team_1_pts_stats['mean'] + team_2_pts_stats['mean']
    ds = team_1_pts_stats['std'] + team_2_pts_stats['std']
    total_points = [dm - (3 * ds), dm - (2 * ds), dm - (1 * ds), dm, dm + (1 * ds), dm + (2 * ds), dm + (3 * ds)]

    index = ['-3sigma', '-2sigma', '-1sigma', 'mean', '+1sigma', '+2sigma', '+3sigma']

    d_ = {'win_margin_vs': win_margin_vs, 'total_points_vs': total_points_vs, 'total_points_l_5': total_points_lst_5,
          'total_points_l_10': total_points_lst_10, 'total_points_l_15': total_points_lst_15,
    ...
    return stats, vs_df, vs_pts_stats, team_1_log, team_2_log, team_1_pts_stats, \
        team_1_pts_stats_last_15, team_1_pts_stats_last_10, team_1_pts_stats_last_5, team_2_pts_stats, \
        team_2_pts_stats_last_15, team_2_pts_stats_last_10, team_2_pts_stats_last_5


def get_games_on_date(games_df: pd.DataFrame, date: datetime, abbreviation_df: pd.DataFrame):
...
if __name__ == '__main__':
    ...
    easter__teams_, western__teams_, team__log_dict_ = team_ranker(teams__df, seaso_n_, season__type_, {})
    ...
    games = get_games_on_date(games_df=basketball_ref_games, date=datetime(year=2023, month=3, day=25),
    ...
    for game in games:
        ...
        sts, vdf, vps, t1, t2, t1ps, t1p15, t1p10, t1p5, t2ps, t2p15, t2p10, t2p5 = \
            get_stats(team__1, team__2, team__log_dict_, to_merge__df, basketball_ref_games, teams__df)

        # Create a DataFrame with the data
        data = {'Last head to head dates': vdf.GAME_DATE.values, f'Last {team__1} head to head w/L': vdf.WL_1.values,
                'Last head to head point differences': vdf.PTS_DIFF.values,
                'Last head to head point differences skew': skew(vdf.PTS_DIFF.values, axis=0, bias=True),
                'Lst head to head point differences kurtosis': kurtosis(vdf.PTS_DIFF.values, axis=0, bias=True),
        ...
        emailer += f"<p>Last head to head point differences: {vdf.PTS_DIFF.values} \n</p>"
        ...
        emailer += s
        data[f'{team__1} Season pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__1} Last 15 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__1} Last 10 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        emailer += f"<p>*******************************************************************************************</p>"
        data[f'{team__1} Last 5 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__2} Season pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__2} Last 15 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__2} Last 10 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}
        ...
        emailer += s
        data[f'{team__2} Last 5 pts scaled mean'] = {'mean': a, 'stdev': b, 'skew': c, 'kurtosis': d}

        emailer += f"<p>*******************************************************************************************</p>"
        try:
            diff_cdf = norm.cdf([i for i in range(41)], vps['mean_diff'], vps['std_diff'])
        ...
        head_to_head_df['pts_diff_skew'].iloc[-1] = data['Last head to head point differences skew']
        head_to_head_df['pts_diff_kurtosis'].iloc[-1] = data['Lst head to head point differences kurtosis']

        ii = 'last_head_to_head_stats'
        head_to_head_stats_df = pd.DataFrame({f'{team__1}': [data[ii]['mean_1'], data[ii]['std_1'],
                                                             data[ii]['median_1'], data[ii]['skew_1'],
                                                             data[ii]['kurtosis']],
                                             f'{team__2}': [data[ii]['mean_2'], data[ii]['std_2'], data[ii]['median_2'],
                                                            data[ii]['skew_2'], data[ii]['kurtosis_2']]},
                                             index=['mean', 'std', 'median', 'skew', 'kurtosis'])

        scaled_points_stats_df = pd.DataFrame([data[f'{team__1} Season pts scaled mean'],
                                               ...
                                               data[f'{team__2} Last 5 pts scaled mean']]).T
        ...
        score_diff_probability_df = pd.DataFrame(data['Score Difference Probability'], index=['probability']).T
        resy.append({'head_to_head_df': head_to_head_df, 'head_to_head_stats_df': head_to_head_stats_df,
